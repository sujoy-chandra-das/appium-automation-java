package base;

public class Locators {

    // Declare the variables with proper data types
    public static String dropdown_CountryId = "com.androidsample.generalstore:id/spinnerCountry";
    public static String text_box_NameId = "com.androidsample.generalstore:id/nameField";
    public static String checkbox_Gender_Xpath = "/html/body/div[1]/div/div[2]/div[1]/div[2]/div/div/div/div/div[23]";
    public static String Button_LetsShop_Id = "com.androidsample.generalstore:id/btnLetsShop";


    public static String element_CountryXpath = "//div[@class='_highlighter-box_88g86_381' and @style='left: 59.3444px; top: 509.056px; width: 235.2px; height: 59.3444px;']";
    public static String title_Products_Xpath = "//android.widget.TextView[@resource-id=\"com.androidsample.generalstore:id/toolbar_title\"]";
    public static String AddtoCart1_Xpath = "(//android.widget.TextView[@resource-id='com.androidsample.generalstore:id/productAddCart'])[1]";
    public static String AddtoCart2_Xpath = "(//android.widget.TextView[@resource-id='com.androidsample.generalstore:id/productAddCart'])[2]";
    public static String Button_Cart_ID= "com.androidsample.generalstore:id/appbar_btn_cart";
    public static String Title_CartID= "com.androidsample.generalstore:id/toolbar_title";
    public static String Checkbox_Cart_Xpath= "//android.widget.CheckBox[@text=\"Send me e-mails on discounts related to selected products in future\"]";
    public static String Button_CheckoutId = "com.androidsample.generalstore:id/btnProceed";
    public static String Title_AfterCheckout_Id = "com.androidsample.generalstore:id/webView";
    public static String SearchBar_Id = "com.androidsample.generalstore:id/webView";
    public static String Cart1Display_xpath ="//android.widget.TextView[@resource-id=\"com.androidsample.generalstore:id/productName\" and @text=\"Air Jordan 4 Retro\"]";
    public static String Cart2Display_xpath ="//android.widget.TextView[@resource-id=\"com.androidsample.generalstore:id/productName\" and @text=\"Air Jordan 1 Mid SE\"]";


}

